...
public class NewDyeProductUIManager : MonoBehaviour
{
    ...
    private void StartAnimations()
    {
        ...
        // 确保初始状态下三个元素完全不可见
        SetElementVisible(banlan, false);
        SetElementVisible(lime, false);
        SetElementVisible(rawmaterialsIntroduce, false);

        // 执行动画
        // Bucket向左平移400单位，时长1秒
        // 在bucket移动完成后才开始其他元素的动画
        bucket.transform.DOMoveX(bucketInitialPos.x - 400, 1f).OnComplete(() => {
    ...
    private void SetElementVisible(GameObject element, bool visible)
    {
        // 设置元素的可见性
